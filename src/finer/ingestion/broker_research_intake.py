...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # C6: broker raw PDFs (and the meta JSONL) normally live on an external
    # volume. When the meta is unreachable AND that volume is unmounted, skip the
    # whole broker intake with a warning alert instead of erroring — the real
    # "external disk unplugged" case. A reachable meta (volume up, or a test /
    # local meta on the internal disk) proceeds normally; a genuinely missing
    # meta with the volume up is still a hard error (typo path).
