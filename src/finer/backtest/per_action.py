...
P&L，正=判断对"):
  ...
  - exit rules: resolved per action from F4 policy hints stored in
    ``TradeAction.metadata`` (``stop_loss_pct`` / ``take_profit_pct`` /
    ``max_holding_days``), falling back to the module defaults below
    (stop -10%, take-profit +20%, time exit 30 calendar days); else
    end-of-data (ExitReason.END_OF_PERIOD).
...
def exit_rules_of(action: TradeAction) -> ExitRules:
    ...
    days = meta.get("max_holding_days")
...
def evaluate_action(
    ...
    return (
        BacktestResult(
            ...
            backtest_period=f"{entry_day.isoformat()} — {exit_day.isoformat()}",
        ),
        None,
    )
