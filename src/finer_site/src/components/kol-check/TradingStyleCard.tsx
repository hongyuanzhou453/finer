 ...
 * observed 由真实 F5 归属 action 统计得出；任一层缺失时渲染降级态。
 * Ported verbatim from the Finer OS dashboard (import repointed to the local
 * contract slice).
 */
...
import type {
  ...
  TradingStyleProfile,
} from "@/demo/kol-check/contracts-style";
  ...
  if (!profile || (!profile.declared && !profile.observed)) {
    return (
      <div className="editorial-panel rounded-sm p-4 text-[12px] text-[var(--ink-soft)]">
        暂无交易风格画像 — declared 层待人工标注，observed 层待该 KOL 归属 action 数据积累。
      </div>
    );
  }
