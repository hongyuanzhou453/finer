...
1. **主源用 Wechat2RSS 公开 feed**——零账号风险，把目标财经号通过 GitHub issue 申请上架（实测 8-01 提交当日即受理）。注意其授权条款限个人研究使用，产出结论不可对外分发原文。
2. **兜底用 `--url-file` 手工队列**——永不失效，适合重点号。
3. **要规模且愿付费时再上极致了**——零账号风险，但上线前先买最小额度对 3 个号跑 T+1 对账。
4. **不建议**：任何 Hook / iPad 协议 / 群控 SDK（唯一会真封微信号的路线）；修旧的 fakeid 链路（已死）；微信读书官方 Gateway（已实测证伪）。
