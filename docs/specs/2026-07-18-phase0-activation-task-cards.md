...
- ✅ **2026-07-18 完成**（详见 `docs/specs/2026-07-18-c4-scheduler-shell.md`；已推 main）：`schemas/heartbeat.py`(HeartbeatState 后端专用) + `src/finer/ops/`(seed 包 + `heartbeat.py` write/read，原子+best-effort) + 两份 launchd plist(`configs/launchd/com.finer.{pipeline-drive,feishu-watch}.plist`，RunAtLoad+KeepAlive+ThrottleInterval) + 两个 wrapper(`scripts/ops/run_*.sh`，走 `.venv`、日志按天、+x)。`cli.py` watch 循环每 pass 末写心跳(`_drive_heartbeat_state` 抽为模块级可单测)，复用 PR#8 fcntl 锁未重复实现。**心跳只给 pipeline-drive**(单文件 heartbeat.json，两 job 会互覆盖)。实证：`--watch 2 --dry-run` → heartbeat.json cycles 递增+lock_holder；并发两实例 → 恰一个 `skipped_locked=true`；plutil+strict plistlib 双过。测试 +20；full suite **3645 passed / 22 skipped**（基线 3625+20）。
  - **R4（C4 前必修，已修）**：`--stages` 加 argparse `type=_stages_arg` 解析期校验（对齐 `--channel` choices）——非法值现 `SystemExit(2)`+可读消息，不再裸 traceback 逃逸或在 `--watch`/launchd KeepAlive 下 respawn-loop；命令函数再兜程序化字符串 stages。见 §2.5 R4 行。
  - **dry-run 副产**：心跳 `last_pass_stats` 显示 broker `f1_ran=0 / f2_ran=2659 / f5_ran=1639`——再次印证 C3：broker F1 已抽干，激活前沿是 F2/F5。
...
- ✅ **2026-07-18 完成（前向代码 + 存量回填，用户拍板方案 A）**（详见 `docs/specs/2026-07-18-c7-signal-class-f4-persistence.md`；code 已推 main）：`TradeAction.signal_class`（`SIGNAL_CLASS_LITERAL` 单一真相源 + contracts.ts `SignalClass` + drift REGISTRY 登记，26 enums 绿）；`action_composer.derive_signal_class`（从 `intent.actionability=="recommendation"` 派生，单一构造点设 top-level + metadata，四要素断言不破坏）；`drive_broker_recommendations.py --execute` 落盘 F4 `PolicyMappingResult`。顺带修 C6 test-hygiene（driver 测试独立于外置盘挂载态）+ intake guard 精化。
  - ⚠️ **现状偏离卡假设 88×**：卡写「19 条 bri」，**live 实为 1,773 条**（scorecard 放量）。验收 `grep bri` 口径失效（F4 按 UUID policy_id 命名）→ 改用「bri action 的 policy_id 有对应 F4 + /audit 三段非空」。
  - ✅ **存量 1,773 就地回填（方案 A，用户确认 dry-run 后执行）**：`scripts/backfill_bri_signal_class_f4.py --execute`（**先全量备份** `data/F5_executed.bak-20260719-034713-bri-backfill-674d69ce`）→ 1,773 条补 `signal_class=broker_recommendation` + 按各自现有 `policy_id` 重建 1,773 个 F4 artifact（沿用 id，保住 scorecard 1,099 结算结果）。**F4 覆盖 1773/1773=100%**；实测一条 bri action `/audit` bundle intent/policy/trade_action 三段全非空、policy_id 对齐。
  - **验证**：drift 绿 / tsc CLEAN / npm build 绿 / pytest **3689 passed, 22 skipped**（含 backfill 3 测试，全 mock 无真数据）。回填幂等、可逆（备份在）。
...
#### 🔴 2026-07-25 选批策略修正（架构收口轮 D1 实测，**开工前必读**）

D1 的 T9 适配器干跑暴露了一个此前未知的语料格局，它直接改写本卡的「导入下一批」含义。全量对账实测：

| 池 | 唯一 filepath（json_ok） | 已在 F0 | 未导入 |
|---|---|---|---|
| T3（个股结构化） | 6,631 | 6,441（**97.1%**） | 190 |
| T9（行业/宏观深抽） | 18,747 | 13（**0.07%**） | **18,734** |
| T3 ∩ T9 | **0** | — | — |

三条结论：

1. **T3 与 T9 覆盖的研报集合完全不相交**（交集 0）。现有 F0 语料 ≈ T3 池，所以现役 3,187 个 bri action 全部来自个股研报；行业/宏观研报**一份都没进过 F0**。
2. 因此「下一批未建档研报」实际上就是**从 T9 池里取**——C10 金丝雀天然是 T9 池的金丝雀，产出的是 `t9i_` sector intent（走 D1 适配器 + sector_proxies.yaml），**不是** bri。原卡「新增 bri action 全部过 C8 审计」的验收口径需要相应扩展。
3. 未导入候选池 T9∪T3 = **18,924 份**，抽样 300 份磁盘存在率 300/300，无缺文件风险。

**修正后的做法**（覆盖原 ①，其余不变）：

- ① 选批**显式按 T9 池取**，不要用「目录序下一批」的隐式口径——否则 1,000 份的构成不可控，报告的外推失真。建议先落一份候选清单文件（T9 池 minus 已导入），再 `--limit 1000` 按清单导入。
- ② 驱动链在 A3（bri 适配器）之外**并跑 D1**：`scripts/drive_t9_sector_intents.py --limit N --execute --drive-f5`（该脚本干跑默认、`--drive-f5` 需 `--execute`，小样本先行）。
- ③ 报告新增两栏：**theme→sector 命中率**与 **`sector_proxy_not_configured` 拒绝分布**——后者就是 `configs/sector_proxies.yaml` 的待补清单，是本批次除吞吐外的第二产出。干跑已知外盘主题（美国/全球/…）会被适配器主动拒绝，这部分应单独计数、不计入失败率。
- **验收扩展**：新增 bri **与 t9i** action 均过 C8 三向审计；t9i 的 sector→proxy ETF 落点抽样人工核验（proxy 是否真能代表该主题）。
...
| R4 | ~~minor~~ **✅ 已修（随 C4）** | 非法 `--stages` 值以裸 pydantic traceback 逃逸 CLI；`--watch` 下首轮即砸死守护循环（cli.py:207-215 无校验，对照 --channel 有 argparse choices） | **✅ 2026-07-18 已修**：`--stages` 加 argparse `type=_stages_arg`（解析期校验，非法→`SystemExit(2)`+可读消息，对齐 `--channel` choices）+ 命令函数兜程序化字符串 stages；6 测试锁定（含 CLI exit-2）。见 C4 ✅ 行 |
