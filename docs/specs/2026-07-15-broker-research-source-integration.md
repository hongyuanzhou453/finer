...
## 4.97 放量收官 + 记分卡 v2（2026-07-19，独立审计 PASS）

**放量漏斗（911 分钟过夜链）**：4,286 T3 行 → F0 join 4,208 → F3 intent **3,834** → grounding 桥 1,980
（**最大流失级：no_anchor_match 1,835 = 47.9%**）→ F4 全映射（open 1,399/watch 503/reduce 78）
→ F5 canonical 1,754（pseudo_ticker 拒 226）→ 含首批共 **1,773 条现役机构 action**。端到端转化 41.4%。
F1/F2 均 4,170/4,170 零失败。

**全量结算**：settled 1,099（V 424 / F 675）+ 窗口未走完 184 + neutral 465 + no_data 25。
765 unique ticker 价格预热 111s。审计独立比价 7/7 bit-for-bit（含空头赢、跳空 +187% 离谱值定性为真实数据）、
非 bri 与首批终态零回翻（mtime 实证）、窗口后缀零裸 30d。

**记分卡 v2 主表**（n≥10，全表在 scratchpad/broker_scorecard_v2.md）：
德银 55.2%(29) > Cantor 46.4%(28) > 摩根大通 42.7%(96) > 花旗 42.0%(88) > 伯恩斯坦 39.7%(68)
> 摩根士丹利 38.2%(170) > 瑞银 36.6%(202) > 杰富瑞 35.2%(142) > 高盛 33.8%(157) > 野村 29.4%(17)。
全体 38.6%（424-675），mean ≈ 0。

**三个结构性发现（比单家胜率重要）**：
1. **市场 > 券商**：A股 47.1%(242) vs US 41.1%(672) vs **港股 18.4%(185, stop 率 68.6%)**——市场间极差 28.7pp
   大于券商间 21pp。港股弱势跨月稳定，高盛/杰富瑞 median 卡 -20.5% 主要是港股敞口被批量止损。
2. **38.6% 是参数下的胜率**：败局 80.9% 由 -20% 硬止损截断（91% 聚集 [-25%,-18%]）；target_reached 率仅 22.1%，
   胜局 43% 是到期浮盈。横比券商公平，绝对值不可对外直读。
3. **2026-03 后月度滑坡是右删失伪影**：近月只有下跌票能提前结算（触止损），上涨票还没到目标——184 条
   still-open 翻牌后会向上修正。「2026 券商集体失准」的叙事不成立。摩根士丹利是唯一 bearish(53.3%)
   显著好于 bullish(36.8%) 的大样本券商，其看空票值得单独跟踪。

**新工单**：① 同券商同票同向 action 去重（CRVS +187% 被两份相同研报重复计分）；② no_anchor_match 1,835
是下一个最大回收池（F2 registry 第二轮）；③ SQLite index 169 条陈旧孤儿行（cache 卫生）；④ pending 674 待
窗口走完后重结算（settle 幂等，重跑即可）。

## 4.98 全语料 OCR 收官 + 记分卡 v3（2026-07-22，独立审计 PASS）

**文本覆盖 1% → 99%**：全语料 26,069 份中 **25,815 份现有正文**（剩 254 份真损坏无法恢复）。
路径：T2 vision OCR（key2 涓流 2025 池 + key1 接力 2026 池 1,908 份零失败）+ 零 token pypdf mop-up（2026 遗漏 1,061）。
**T4 多实体全量 17,032 份**宏观/行业研报 → `burn_t4.jsonl`（~1.1 亿 token，宏观层第一份结构化资产）。

**双 key 调度教训**：一个 key 同时只挂一条 reasoning 重载线（mimo-v2.5 单 key TPM ~20-30 万）。
key1 宽水管独占 T4/sweep，key2 细水管跑 c3 OCR 涓流 + 600s 自愈重启。外置盘两次物理断开 →
改本地优先架构（db `.backup` 到本地、分批装载 rsync、产物落本地盘），零数据损失。

**记分卡 v3**（2,616 action / **settled 1,530** / 胜率 **43.6%**，vs v2 的 1,773/1,099/38.6%）：

| 券商 | settled | 胜率 | Δvs v2 | | 券商 | settled | 胜率 | Δ |
|---|---|---|---|---|---|---|---|---|
| 德银 | 36 | **61.1%** | +5.9 | | 花旗 | 131 | 46.6% | +4.6 |
| 伯恩斯坦 | 126 | 50.8% | **+11.1** | | 高盛 | 215 | 40.5% | +6.7 |
| 汇丰 | 12 | 50.0% | +4.5 | | 瑞银 | 297 | 42.8% | +6.2 |
| 摩根大通 | 136 | 47.8% | +5.1 | | 杰富瑞 | 190 | 34.7% | **-0.5** |
| 摩根士丹利 | 231 | 45.9% | +7.7 | | **全体** | **1530** | **43.6%** | **+5.0** |

**v2 三大发现的更大样本复核（关键）**：
1. **「市场 > 券商」被部分证伪**：+431 新结算里 430 是 US（右删失翻牌），US 追平 A股（47.0% vs 47.3%），
   市场梯度坍缩成「.HK 悬崖 vs 其余」。券商极差 28.7pp 现已 ≈ 市场极差 28.9pp——市场不再显著压过券商。
2. **止损截断稳如磐石**：败局止损占比 80.9%→80.0%，-20% 硬止损的结构性伪影跨样本复现。
3. **右删失被证实并放大**：所有近月胜率按 v2 预测方向上修（2026-06 5.3%→13.8%），但近月仍 57-67% pending，删失远未出清。

**⚠️ v3 是摄入中快照**：2,370 份 OCR 新解锁单票研报已 F0 导入（6,529 record），但 F1/F2 仅完成 4,559，
剩 ~1,970 待标准化（续跑中）。F1/F2 完成后 adapter→drive→settle 会再增 action（全程零 token）。
诚实说明：v3 的 +5pp 胜率**主要是 US 子集从右删失翻牌的组合效应，不是券商判断力变好**。

## 4.99 记分卡终版 v4（2026-07-22，全 6,529 F0/F1/F2 语料，零 token，审计 PASS）

F1/F2 在全部 6,529 份 broker record 完成（F0=F1=F2 对齐）后，零 token 物化：
adapter（intent 4,059→**5,852**）→ drive（action 2,616→**3,127**，canonical 100%）→ settle（settled 1,530→**1,839**）。

**记分卡终版**（3,127 action / **settled 1,839** / 胜率 **42.7%**）：

| 券商 | settled | 胜率 | | 券商 | settled | 胜率 |
|---|---|---|---|---|---|---|
| 德银 | 39 | **59.0%** | | 花旗 | 154 | 44.8% |
| 伯恩斯坦 | 140 | 51.4% | | 瑞银 | 356 | 43.0% |
| 美银 | 14 | 50.0% | | 高盛 | 273 | 38.5% |
| 摩根大通 | 167 | 46.1% | | 杰富瑞 | 233 | 36.5% |
| 摩根士丹利 | 268 | 44.8% | | 野村 | 30 | 26.7% |
| 汇丰 | 17 | 41.2% | | **全体** | **1,839** | **42.7%** |

**关键：量增 +20% 未改善质。** 新增 309 结算里增量胜率仅 38.2%，把整卡从 43.6% 压到 42.7%——
与 v2→v3（US 翻牌拉高）方向相反，这一轮翻出来的更多是止损。**这正是右删失的实证**。

**三大发现终检**：
1. **「市场>券商」进一步证伪**：券商极差（德银 59.0 − 野村 26.7 = 32.3pp）**反超**市场极差（27.4pp）。
   市场梯度坍缩为「港股 19.3% 悬崖 vs 其余」，「券商是谁」比「哪个市场」更能解释离散。
2. **止损截断纹丝不动**：stop 占败局 81.4%（三代 80.9/80.0/81.4），止损单 89.9% 落 [-25%,-18%]。
   **42.7% 是 -20% 硬止损的函数,非券商判断力的无偏估计。**
3. **右删失证实并再放大**：近月（2026-03+）60% pending，本轮增量以止损为主印证删失向下修正。

**no_anchor_match 仍主导（82.4%）**：grounding 命中率 0.14%→17.6%（127× 跃升）、no_envelope 222→0，
但绝对 no_anchor_match 升（可运行 intent 翻倍）。根因**不是 F2 缺锚点**，而是券商 target_symbol 多为国际代码
（SN.N/SUNT.SI/6976.T/1339.HK…）未进 F2 resolved_symbol——**F2 国际后缀归一化是下一个最大回收杠杆,零 token**。

## 4.995 F2 归一化真相 + 止损敏感性（2026-07-22，零 token，审计 PASS）

**F2 归一化诊断推翻前提**：全量 2,665 条 no_anchor_match 精确分桶——
桶 A **48.1%（1,281）** F2 envelope 零 resolved_symbol（覆盖缺口，无锚可比）；
桶 B **6.1%（163）** 仅后缀/形态差异（可归一化救回）；桶 C **45.8%（1,221）** 券商正文锚了别的票（救=伪造）。
**94% 是 F2 覆盖缺口(A)+目标未锚定(C)，不是后缀方言。** 纯归一化只能安全救 163 条。
实做：桥接层 loose 匹配（单锚点命中才桥、多锚点歧义拒），grounding 17.6%→22.7%，**action 3,127→3,187（+60，100% canonical）**。
**真天花板**：`entity_registry.matches_tradable_format` 正则 `^\d{4,6}\.(HK|SH|SZ)$|^[A-Z]{1,5}$` 拒绝所有国际后缀
（.T/.OL/.BO/.KS/.AX/.TO），挡掉 ~103 已桥接匹配——共享模块，需独立评审后扩（影响 F5 LLM 路径）。
**结论：no_anchor_match 的真正回收杠杆是 F2 锚点召回（A/C 桶），不是后缀归一化。**

**止损参数敏感性（2,174 条 directional 缓存回放，同批贯穿四列，审计 2/2 手算逐位一致）**：

| 止损档 | 胜率 | median | stop率 | time_exit率 |
|---|---|---|---|---|
| **-20%（现状）** | 44.4% | -4.0% | 40.0% | 25.2% |
| -25% | 47.1% | -1.8% | 30.2% | 31.3% |
| -30% | 48.0% | -1.3% | 21.6% | 37.4% |
| **无止损（纯 180d）** | **49.2%** | -0.5% | 0.0% | 53.3% |

**回答了悬三代的问题**：-20% 打掉的 869 条败局里,**仅 12.1%（105）是"参数误杀"**（放开后收正），88% 是真看错方向。
**"真判断力"胜率上界 ≈ 49.2%**（无止损纯持有），止损参数只解释 ~4.8pp。
**券商研报整体是接近抛硬币、微正期望的信号源；去掉参数噪音后判断力也就将将过半。**
分券商：杰富瑞被止损误伤最重（ΔWin +9.3pp）；野村/BMO 放宽止损也救不动（真判断差）。
**战术含义：-20% 对 12 个月目标价太紧（等于用日内风控管年度观点），-30% 是更匹配 horizon 的折中。**

## 4.996 CRVS 去重（2026-07-22，零 token，审计 PASS）

真重复判据=同 creator+ticker+direction+入场日+return_pct 全等（同票不同时间的多次评级是合法的，不动）。
**190 组真重复、206 条多余 action 标 `metadata.superseded_by`（不删文件，红线）**，审计独立核实：
0 结构破坏、0 保留者误标、226 组合法多次评级全部保留未误伤。

**根因**：同一篇研报被重复接入——content_id 是文件字节 sha256，但同内容报告存成了字节略异的两个 PDF
（券商重发/两处归档），F0 内容哈希没抓到。数据质量观察，非 pipeline bug。

**反直觉修正**：只剔 CRVS 时 mean 1.79%→1.71%（消 +187% 虚高），但剔全部 206 条 mean 反**升**到 **1.88%**——
因为 206 条多余项多为亏损单被双记，此前压低了均值。**去重同时修正了 CRVS 正向虚高 + 大量亏损单负向虚低。**
修正后：settled 2,177→**1,971**，胜率 44.37%→**44.50%**，mean +1.79%→**+1.88%**。

未决：SQLite index superseded 列待授权加（schema 变更红线）；记分卡消费端需读 `superseded_by` 生效（数据侧已就位）。
